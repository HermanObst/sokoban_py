...
def mover(grilla, direccion):
	...
	if movimiento_no_permitido(grilla,direccion):
		return grilla
...
def movimiento_no_permitido(grilla,direccion):
	"""Dada una grilla y una dirección, devuelve True si el movimiento NO esta permitido"""
	x, y = direccion
	f_j, c_j = posicion_jugador(grilla)

	return (hay_pared(grilla, c_j + x, f_j + y)) or (hay_caja(grilla, c_j + x, f_j + y) and (hay_pared(grilla, c_j + 2*x, f_j + 2*y) or hay_caja(grilla, c_j + x*2, f_j + y*2)))
